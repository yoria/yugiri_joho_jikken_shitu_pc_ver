...
int ERS_Open(int n, int recv_size, int send_size)
{
	...
	wcstombs_s(&ret, comname_char, 11, comname, _TRUNCATE);/**/

	ERSHCOMn = CreateFile(comname_char, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);/**/
...
int ERS_ConfigDialog(int n)
{
	...
	wcstombs_s(&ret, comname_char, 7, comname, _TRUNCATE);/**/

	GetCommConfig(ERSHCOMn,&cc,&size);
	CommConfigDialog(comname_char, HWND_DESKTOP, &cc);/**/
